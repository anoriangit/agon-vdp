...
void copy_font(int id) {
  if (id >= 0 && id < AGON_NUM_FLASH_FONTS) {
    const fontmem_t *fd = &fabgl::AGON_FONTS_MEMORY[id];
    memcpy(fabgl::FONT_AGON_DATA + fd->offset, fd->bitmap, fd->size);
  } else {
...
char get_screen_char(int px, int py) {
  RGB888 pixel;
  uint8_t charRow;
  uint8_t charData[AGON_MAX_FONT_HEIGHT];
  ...
  // (gs) adapted to scan up to height rows but still hard coded to width=8 like the original
  ...
  for (int i = 32; i < 128; i++) {
    if (cmp_char(charData, &fabgl::FONT_AGON_DATA[i * CURRENT_FONT->height], CURRENT_FONT->height)) {
      return i;
    }
  }
  return 0;
...
void vdu_palette() {
  int l = readByte_t();
  if (l == -1) return;  // Logical colour
  int p = readByte_t();
  if (p == -1) return;  // Physical colour
  int r = readByte_t();
  if (r == -1) return;  // The red component
  int g = readByte_t();
  if (g == -1) return;  // The green component
  int b = readByte_t();
  if (b == -1) return;  // The blue component
...
void vdu_sys() {
  ...
  // If mode < 32, then it's a system command  //
    ...
    switch (mode) {
      ...
      case 0X1A:
        {
          // 23, 26, fontnumber, pointsize, xppi, yppi (more parameters possible...)
          // see "Select Font" reference: https://beebwiki.mdfs.net/VDU_23#VDU_23.2C26_-_Select_font
          // This is a non standard modification using fontnumber, size, and xppi/yppi interpreted
          // as x/y resolution (gs)
          // When fontnumber < AGON_NUM_FONTS then we simply select/enable the respective baked in
          // flash memory font (like the deprecated 23,0,160 sequence used to do )
          int id = readByte_t();
          if (id == -1)
            return;
          int size = readByte_t();
          if(size == -1)
            return;
          int width = readByte_t();
          if(width == -1)
            return;
          int height = readByte_t();
          if(height == -1)
            return;
            
          // FLASH resident fonts activation: only id is actually needed (all other arguments are ignored)
          if (id < AGON_NUM_FLASH_FONTS) {
          ...
          } else if (id == AGON_USER_FONT_ID) {

            id = AGON_NUM_FLASH_FONTS;  // the user font is always the last font in the table
            ...
            if(width > 0)
              USER_FONT->width = width;
            if(height > 0)
              USER_FONT->height = height;

            CURRENT_FONT = USER_FONT;

          }
          // need to set_mode() to ensure all required components in fabgl and MOS get updated
          set_mode(videoMode);
        }
        break;
      case 0x1B:
...
void vdu_sys_scroll() {
  ...
  if (extent == -1) return;  // Extent (0 = current text window, 1 = entire screen)
